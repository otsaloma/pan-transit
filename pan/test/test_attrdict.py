...
class TestAttrDict(pan.test.TestCase):

    def setup_method(self, method):
        self.dct = pan.AttrDict(a=1,
                                b=[1, 2, 3],
                                c=dict(a=1, b=2, c=3))

    def test___init____nested(self):
        assert isinstance(self.dct.c, pan.AttrDict)

    def test___coerce(self):
        self.dct.d = [dict(e=1), dict(f=1)]
        assert isinstance(self.dct.d[0], pan.AttrDict)
        assert isinstance(self.dct.d[1], pan.AttrDict)
    ...
    def test___setattr____nested(self):
        ...
        assert isinstance(self.dct.d, pan.AttrDict)
        assert isinstance(self.dct.d.f, pan.AttrDict)
